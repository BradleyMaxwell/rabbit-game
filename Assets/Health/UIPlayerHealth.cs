...
public class UIPlayerHealth : MonoBehaviour // changed how the health hearts are shown on the UI when the player's health changes
{
    ...
    private void RemoveHearts() // go left changing hearts to missing hearts
    {
    ...
    private void AddHearts() // go right changing hearts from missing hearts to hearts, starting from the next heart in the list as that is the first missing heart
    {
    ...
    private void SetupHearts() // called when the scene starts to setup the hearts and position them correctly based on the player's max health
    {
        for (int i = 0; i < playerHealthInfo.MaxHealth; i++)
        {
            ...
            hearts.Add(heart);
            float offset = i * GetComponent<RectTransform>().rect.width; // how much to move the current heart UI element to the right by so each heart is shown to the right of the previous
