...
public abstract class TargetedAbilityManager : AbilityManager // handles logic and information about an enemy's targeted ability needed for the scene
{
    private TargetedAbility targetedAbility; // for using data specific to targeted abilities

    private void Awake()
    {
        if (ability is not TargetedAbility)
        {
            throw new ArgumentException("ability scriptable object must be a targeted ability");
        }
        else
        {
            targetedAbility = (TargetedAbility)ability;
        }
    }
    ...
    {
        float castTimeRemaining = ability.CastTime; // this will go down every frame until the cast is finished
    ...
    public bool IsInRange(Transform target) // returns if the distance between the user and the target is within the range
    {
        ...
        if (Vector3.Distance(userPosition, targetPosition) <= targetedAbility.Range)
        {
            return true;
        }
        else
